...
export default function App() {
  ...
  const [complianceReqs, setComplianceReqs] = useState([
    { id:"r1", name:"PAN Card",          cat:"Identity"  },
    { id:"r2", name:"Aadhaar Card",      cat:"Identity"  },
    { id:"r3", name:"Bank Passbook / Cancelled Cheque", cat:"Financial" },
  ]);
                                ...
                                <div style={{ display:"flex", gap:8, justifyContent:"flex-end" }}>
                                  {isSigned ? (
                                    <>
                                      ...
                                      <button onClick={() => setViewingDoc(doc)} style={{ background:`rgba(var(--p-rgb),.04)`, border:`1px solid rgba(var(--p-rgb),.15)`, borderRadius:8, padding:"6px 12px", fontSize:11, fontWeight:600, color:C.p, cursor:"pointer", display:"flex", gap:6, alignItems:"center" }}>{IconEye} View</button>
                                    ...
                                    <>
                                      <button onClick={() => setViewingDoc(doc)} style={{ background:`rgba(var(--p-rgb),.04)`, border:`1px solid rgba(var(--p-rgb),.15)`, borderRadius:8, padding:"6px 12px", fontSize:11, fontWeight:600, color:C.p, cursor:"pointer", display:"flex", gap:6, alignItems:"center" }}>{IconEye}</button>
                                    ...
                                    <>
                                      ...
                                      }} style={{ background:`rgba(var(--p-rgb),.1)`, border:`1px solid rgba(var(--p-rgb),.2)`, borderRadius:8, padding:"6px 16px", fontSize:11, fontWeight:700, color:C.p, cursor:"pointer", display:"flex", alignItems:"center", gap:6 }}>{IconSend} Send</button>
                                    </>
                                  )}
                                </div>
                              </td>
                            ...
                            <tr key={i} onClick={() => setComplianceEmpDetail({ emp: row.emp, mode:"overview" })}
                              ...
                              <td style={{ padding:"14px 20px" }}>
                                <button onClick={ev=>{ ev.stopPropagation(); setComplianceDocFilter({ emp: row.emp, type:"missing" }); }}
                                  style={{ fontWeight:700, color: row.missing.length>0 ? "#c0392b" : C.sub, background: row.missing.length>0?"`rgba(192,57,43,.1)`":C.surf, border:"none", borderRadius:6, padding:"3px 10px", cursor: row.missing.length>0?"pointer":"default", fontSize:13 }}>
                                  {row.missing.length}
                                </button>
                              </td>
                              <td style={{ padding:"14px 20px" }}>
                                {row.stat === "Complete" ? (
                                  ...
                                  <span style={{ fontSize:11, fontWeight:700, color:"#c0392b", background:`rgba(192,57,43,.1)`, padding:"4px 12px", borderRadius:99 }}>OVERDUE</span>
                                ) : (
                                  <span style={{ fontSize:11, fontWeight:700, color:"#a07840", background:`rgba(160,120,64,.1)`, padding:"4px 12px", borderRadius:99 }}>PENDING</span>
                                )}
                              </td>
                              <td style={{ padding:"14px 20px", textAlign:"right" }}>
                                <button onClick={ev=>{ ev.stopPropagation(); setComplianceEmpDetail({ emp: row.emp, mode:"add" }); }}
                                  style={{ background:`rgba(var(--p-rgb),.1)`, color:C.p, border:`1px solid rgba(var(--p-rgb),.2)`, borderRadius:8, padding:"6px 14px", fontSize:11, fontWeight:700, cursor:"pointer" }}>
                                  + Add Doc
                                </button>
                              </td>
                            </tr>
                            ...
                            {complianceReqs.map(req => (
                              <div key={req.id} style={{ display:"flex", alignItems:"center", gap:10, background:C.surf, padding:"10px 14px", borderRadius:10, border:`1px solid ${C.bdr}` }}>
                                ...
                                <button onClick={() => setComplianceReqs(prev => prev.filter(r => r.id !== req.id))}
                                  style={{ background:"none", border:`1px solid rgba(220,38,38,.25)`, borderRadius:6, padding:"4px 10px", fontSize:11, fontWeight:600, color:"#c0392b", cursor:"pointer" }}>
                                  Remove
                                </button>
                              </div>
                            ))}
                          ...
                          <div style={{ padding:"16px 24px 24px" }}>
                              ...
                              <div style={{ marginBottom:20 }}>
                                ...
                                {missing.map(r => (
                                  <div key={r.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 14px", background:`rgba(160,120,64,.06)`, borderRadius:8, border:"1px solid `rgba(160,120,64,.22)`", marginBottom:6 }}>
                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#c0392b" strokeWidth="2.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                                    <div style={{ flex:1, fontWeight:600, fontSize:13, color:"#c0392b" }}>{r.name}</div>
                                    <span style={{ fontSize:11, color:"#c0392b" }}>{r.cat}</span>
                                  </div>
                                ))}
                              </div>
                            )}
                            ...
                            {latestDocs.length > 0 && missing.length > 0 && (
                              <div style={{ background:`rgba(160,120,64,.06)`, border:"1px solid `rgba(160,120,64,.25)`", borderRadius:10, padding:"14px 16px", marginBottom:16 }}>
                                <div style={{ fontSize:11, fontWeight:700, color:"#c0392b", marginBottom:12, letterSpacing:.4 }}>MARK FROM PROFILE DOCUMENTS</div>
                                <div style={{ fontSize:12, color:"#c0392b", marginBottom:12, lineHeight:1.5 }}>
                                ...
                                {missing.map(r => (
                                  <div key={r.id} style={{ marginBottom:10 }}>
                                    <div style={{ fontSize:12, fontWeight:600, color:"#c0392b", marginBottom:5 }}>{r.name}</div>
                                    <div style={{ display:"flex", gap:8 }}>
                                      ...
                                      }} style={{ padding:"8px 14px", background:"#8a6530", color:C.wht, border:"none", borderRadius:8, fontSize:12, fontWeight:700, cursor:"pointer", flexShrink:0 }}>
                                        Mark
                                      </button>
                                    </div>
                                  </div>
                            ...
                            {docs.length === 0 ? (
                              ...
                              <div key={r.id} style={{
                                ...
                                {type === "submitted"
                                  ...
                                  : <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#c0392b" strokeWidth="2.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                                }
                                <div>
                                  <div style={{ fontWeight:600, fontSize:13 }}>{r.name}</div>
